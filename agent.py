    ...
    def createQ(self, state):
        ...
        if self.learning:
            if not self.Q.has_key(state):
                self.Q[state] = dict(zip(Environment.valid_actions, [0.0]*len(Environment.valid_actions)))
...
def run():
    ...
    env.set_primary_agent(agent,  enforce_deadline = True) ## env.set_primary_agent(agent) modified for question 3
